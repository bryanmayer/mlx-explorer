...
writeLines(
  c("// Auto-generated by R/generate-explorer-data.R — do not edit manually",
    glue("// Generated: {Sys.time()}"),
    "",
    paste0("window.MODEL_DATA = ", json_str, ";")),
  out_path
)

n_models <- length(registry)
message(glue("Wrote {n_models} model(s) to analysis/model-data.js"))

# ── Cache-bust the HTML script tag ────────────────────────────────────────────
# Updates model-explorer.html so the browser always fetches the fresh file.
...
if (file.exists(html_path)) {
  ...
  html   <- str_replace(html,
    'src="model-data\\.js(?:\\?v=[^"]*)?">',
    glue('src="model-data.js?v={ts_str}">')
  )
  writeLines(html, html_path)
  message(glue("Updated model-explorer.html cache-bust to v={ts_str}"))
}
